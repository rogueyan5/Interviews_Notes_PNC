...
## 46.野指针和内存泄露有什么区别？
内存泄漏是指使用动态分配函数或关键字申请的内存空间，在使用完毕后未释放，导致一直占据该内存单元，不能被任何程序再次使用，知道程序结束，即是所谓的内存泄露。

表现行为是程序运行时间越长，占用内存越多，最终用尽所有可用内存，造成系统崩溃。

内存泄漏的原因是通过库函数或关键字申请的内存空间未被正确释放。


野指针是指指向不可用内存区域的指针，通常对这种指针进行操作，会导致发生不可预知的错误。

野指针的导致原因是指针变量没有被初始化，任何指针变量刚被创建时不会自动初始化而是随机的，此时对未经初始化的指针进行访问，就可能访问到不合法的内存<br>
第二种情况就是指针p指向的内存被free或者delete之后未置为nullptr，这样就导致内存空间已经释放了，但指针仍然记录了那块内存空间的地址，访问时就会出错。
